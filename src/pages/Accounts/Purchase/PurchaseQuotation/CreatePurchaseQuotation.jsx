...
import { createPurchaseQuotationEffect, updatePurchaseOrderEffect, updatePurchaseQuotationEffect } from "../../../../redux/Account/Purchase/PurchaseQuotation/PurchaseQuotationEffects";
import { DevTool } from "@hookform/devtools";
import SearchableSelect1 from "../../../../UI/Select/selectBox";

export default function CreatePurchaseQuotation() {
    ...
    const { register, formState: { errors }, handleSubmit, setValue, watch, reset, trigger, control } = mainFormMethods;
    ...
    useEffect(() => {
        if (location?.state) {
            ...
            setSelectedVendor({
                ...
                label: `${lead.vendor_name} - ${lead.vendor_id}`,
            });

            // Prefill form fields with lead data
            setValue("uuid", lead?.uuid || "");
            setValue("vendor", lead?.vendor_id || "");
            ...
            setValue("is_address", lead?.is_billing === "true"); if (location.state.sales_quo_material) {
                const transformedData = transformData(location.state.sales_quo_material);
    ...
    const breadcrumbItems = [
        ...
        { id: 2, label: "Purchase", link: "/user/accounts/purchase/purchase-quotation" },
        { id: 3, label: "Create Purchase Quotation" }
    ];
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            console.log("itemData, unitData, ", vendorData);
            setItemList(
    ...
    const addMasterHandler = async (data) => {
        console.log("addMasterHandler", data)
        ...
        if (location.state) {
            ...
            apiEndpoint = updatePurchaseQuotationEffect(payload); // Ensure this function is available
        } else {
            const payload = {
                // cust_id: data?.cust_id || "",
                vendor_id: data?.vendor || "",
                pur_quo_no: data?.pur_quo_no || "",
            ...
            apiEndpoint = createPurchaseQuotationEffect(payload);
        }
                                    ...
                                    <div className="col-span-2">
                                        ...
                                        <SearchableSelect
                                          errors={errors}
                                            ...
                                            setValue={setValue}
                                            defaultid={watch('vendor')}                                            // defaultValue={vendorList?.find((vendor) => vendor?.value === watch('vendor'))} // Match the value
                                        defaultValue={watch('vendor')} // Match the value)} // Match the value
                                        />
                                    </div>
                                    <div className="col-span-2">
