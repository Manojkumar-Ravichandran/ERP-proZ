...
import { cancelPurchaseOrderEffect, listPurchaseQuotationEffect, purchaseQuotationConvertInvoiceEffect, purchaseQuotationConvertOrderEffect } from "../../../../redux/Account/Purchase/PurchaseQuotation/PurchaseQuotationEffects";
import { pdfMailQuotationEffect, pdfWhatsappQuotationEffect, purchaseQuotationPdfEffect } from "../../../../redux/Account/Purchase/PdfWhatsapp";
...
export default function PurchaseQuotation() {
  ...
  const fetchLeadList = async () => {
    setLoading(true);
    try {
      const response = await listPurchaseQuotationEffect({
  ...
  const columnDefs = [
    { headerName: "Quotation Number", field: "quotation_id", unSortIcon: true, },
          ...
          >
            ...
            {params?.data?.status === 1 &&
              ...
              <span
                ...
                onClick={() => {
                  navigate("/user/accounts/purchase/create-purchase-quotation", { state: { ...params.data } });
                }}
              >
  ...
  const handleAction = async (action, params, master) => {
    if (action === "order" && params.data) {
      try {
        const response = await purchaseQuotationConvertOrderEffect({ uuid: params.data.uuid });
        setToastData({
          show: true,
      ...
      try {
        const response = await purchaseQuotationConvertInvoiceEffect({ uuid: params.data.uuid });
        setToastData({
          show: true,
          message: response?.data?.message,
          type: response?.data?.status
      ...
      try {
        const response = await pdfWhatsappQuotationEffect({ uuid: params.data.uuid });
        setToastData({
          show: true,
          message: response?.data?.message,
          type: response?.data?.status
      ...
      try {
        const response = await pdfMailQuotationEffect({ uuid: params.data.uuid });
        setToastData({
          show: true,
          message: response?.data?.message,
          type: response?.data?.status
      ...
      try {
        const response = await purchaseQuotationPdfEffect({ uuid: params.data.uuid });
  ...
  const cancelHandler = async (data) => {
    try {
      const response = await cancelPurchaseOrderEffect({ ...data, uuid: selectedUser?.uuid }); // Replace with your actual API endpoint
      setToastData({
        show: true,
        message: response?.data?.message,
        type: response?.data?.status
        ...
        <div className=" rounded-lg pe-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <IconButton
              ...
              icon={icons.plusIcon}
              onClick={() => {
                navigate("/user/accounts/purchase/create-purchase-quotation");
              }}
            />
          </div>
        </div>
