...
import { purchaseOrderCancelEffect, purchaseOrderConvertInvoiceEffect, purchaseOrderListEffect,  } from "../../../../redux/Account/Purchase/PurchaseOrder/PurchaseOrderEffects";
import { purchaseOrderMailPdfEffect, purchaseOrderPdfEffect, purchaseOrderPdfWhatsappEffect } from "../../../../redux/Account/Purchase/PdfWhatsapp";
...
export default function PurchaseOrder() {
  ...
  const fetchLeadList = async () => {
    setLoading(true);
    try {
      const response = await purchaseOrderListEffect({
  ...
  const columnDefs = [
    { headerName: "Order Number", field: "order_id", unSortIcon: true, },
            ...
            {params?.data?.status === 2  ? (
              <div className="flex justify-center items-center">
                <button
                  ...
                  onClick={() =>
                    navigate("/user/accounts/purchase/purchase-invoice")
                  }
                  title={params.data?.status === 2 && 'Invoice'}
                >
                  Converted...
                </button>
              </div>
          ...
          >
            ...
            {params?.data?.status === 1 &&
              ...
              <span
                ...
                onClick={() => {
                  navigate("/user/accounts/purchase/create-purchase-order", { state: { ...params.data } });
                }}
              >
 ...
 const handleAction = async(action, params, master) => {
        if (action === "invoice" && params.data) {
          try {
            const response = await purchaseOrderConvertInvoiceEffect({ uuid: params.data.uuid });
          ...
          setSelectedUser(params.data);
            setIsCancelModal(true);
              ...
              try {
                const response = await purchaseOrderPdfWhatsappEffect({ uuid: params.data.uuid });
                setToastData({
                  show: true,
                  message: response?.data?.message,
                  type: response?.data?.status
              ...
              try {
                const response = await purchaseOrderMailPdfEffect({ uuid: params.data.uuid });
                setToastData({
                  show: true,
                  message: response?.data?.message,
                  type: response?.data?.status
              ...
              try {
                const response = await purchaseOrderPdfEffect({ uuid: params.data.uuid });
  ...
  const cancelHandler = async (data) => {
    try {
        const response = await purchaseOrderCancelEffect({...data,uuid:selectedUser?.uuid});
        setToastData({
          show: true,
          message: response?.data?.message ,
          type: response?.data?.status
        ...
        <div className=" rounded-lg pe-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <IconButton
              ...
              icon={icons.plusIcon}
              onClick={() => {
                navigate("/user/accounts/purchase/create-purchase-order");
              }}
            />
          </div>
        </div>
