...
import { purchaseReturnCreateEffect, purchaseReturnInvoiceDropdownEffect, purchaseReturnInvoiceItemEffect, purchaseReturnUpdateEffect } from "../../../../redux/Account/Purchase/PurchaseReturn/PurchaseReturnEffects";
...
export default function CreatePurchaseReturn() {
    ...
    useEffect(() => {
        if (location?.state) {
            ...
            setValue("vendor_name", lead?.vendor_name || "");
    ...
    const breadcrumbItems = [
        ...
        { id: 2, label: "Return", link: "/user/accounts/purchase/purchase-return" },
        { id: 3, label: "Create Purchase Return" }
    ];
    ...
    const addMasterHandler = async (data) => {
        ...
        if (location.state) {
            ...
            apiEndpoint = purchaseReturnUpdateEffect(payload); // Ensure this function is available
        } else {
            const payload = {
            ...
            apiEndpoint = purchaseReturnCreateEffect(payload);
        }
        ...
        try {
            ...
            if (response?.data?.status === "success") {
                ...
                navigate('/user/accounts/purchase/purchase-return');
            } else {
                setToastData({
                    show: true,
                    message: response?.data?.message,
    ...
    useEffect(() => {
        (async () => {
            try {
                let { data } = await purchaseReturnInvoiceDropdownEffect();
    ...
    //     setValue("vendor_name", selectedOption.cust_id|| "");
    ...
    const handleInvoiceChange = async (selectedOption) => {
        ...
        setValue("vendor_name", selectedOption.vendor_name || "");
        setValue("invoice_date", selectedOption.date);
        setValue("lead_id", selectedOption.lead_id);
        setValue("invoice_id", selectedOption.value);
        try {
            const response = await purchaseReturnInvoiceItemEffect({ invoice_id: selectedOption.value }); 
            setSelectedItemList(transformData(response?.data?.data));
        } catch (error) {
            console.error("Error fetching invoice data:", error);
        }
