...
// import { createSaleInvoiceEffect, updateSaleInvoiceEffect } from "../../../../redux/Account/Sales/SaleInvoice/SaleInvoiceEffects";
...
//             // Prefill form fields with lead data
...
//             setValue("is_address", lead?.is_billing === "true"); 
//             if (location.state.sales_invoice_material) {
//                 const transformedData = transformData(location.state.sales_invoice_material);
...
//         { id: 2, label: "Invoice", link: "/user/accounts/sale/sale-invoice" },
//         { id: 3, label: "Create Sale Invoice" }
//     ];
//     useEffect(() => {
...
//                 sales_invoice_material: selectedItemList.map(item => ({
...
//                 })),
//             };
//             apiEndpoint = updateSaleInvoiceEffect(payload); // Ensure this function is available
//         } else {
//             const payload = {
...
//                 })),
//             };
//             apiEndpoint = createSaleInvoiceEffect(payload);
//         }
...
//                     type: 'success'
//                 });
//                 navigate('/user/accounts/sale/sale-invoice');
//             } else {
//                 setToastData({
//                     show: true,
//                     message: response?.data?.message,
...
//                             <div>
//                                 <p className='text-lg font-semibold '>Invoice Details</p>
...
import { createPurchaseInvoiceEffect, updateSalesOrderEffect } from "../../../../redux/Account/Purchase/PurchaseInvoice/PurchaseInvoiceEffects";
...
export default function CreatePurchaseInvoice() {
    ...
    useEffect(() => {
        if (location?.state) {
            ...
            setValue("date", lead?.date);
            setValue("is_address", lead?.is_billing === "true");
             if (location?.state?.invoice_material) {
                const transformedData = transformData(location.state.invoice_material);
                setSelectedItemList(transformedData);
            }
        } else {
            setSelectedItemList([]);
    ...
    const breadcrumbItems = [
        { id: 1, label: "Home", link: "/user" },
        { id: 2, label: "Invoice", link: "/user/accounts/purchase/purchase-invoice" },
        { id: 3, label: location?.state ? "Edit Purchase Invoice" : "Create purchase Invoice" }
    ];
    ...
    const addMasterHandler = async (data) => {
        ...
        if (location.state) {
            ...
            apiEndpoint = updateSalesOrderEffect(payload); // Ensure this function is available
        } else {
            const payload = {
                vendor_id: data.vendor_id,
                date: data.date,
            ...
            apiEndpoint = createPurchaseInvoiceEffect(payload); // Ensure this function is available
        }
