...
import { getSaleOrderListEffect, MailShareEffect, PdfCustomerEffect, rejectSaleOrderEffect, SQToInvoiceEffect, WhatsAppShareEffect } from "../../../../redux/Account/Sales/SaleOrder/SaleOrderEffects";
...
export default function SaleOrder() {
  ...
  const fetchLeadList = async () => {
    setLoading(true);
    try {
      const response = await getSaleOrderListEffect({
          ...
          >
            ...
            {params?.data?.status === 1 &&
              ...
              <span
                ...
                onClick={() => {
                  navigate("/user/accounts/sale/create-sale-order", { state: { ...params.data } });
                }}
              >
  ...
  const handleAction = async (action, params, master) => {
    ...
    if (action === "invoice" && params.data) {
      try {
        const response = await SQToInvoiceEffect({ uuid: params.data.uuid });
      ...
      try {
        const response = await WhatsAppShareEffect({ uuid: params.data.uuid });
        setToastData({
          show: true,
          message: response?.data?.message,
          type: response?.data?.status
      ...
      try {
        const response = await MailShareEffect({ uuid: params.data.uuid });
        setToastData({
          show: true,
          message: response?.data?.message,
          type: response?.data?.status
  ...
  const cancelHandler = async (data) => {
    try {
      const response = await rejectSaleOrderEffect({ ...data, uuid: selectedUser?.uuid });
      setToastData({
        show: true,
        message: response?.data?.message,
        type: response?.data?.status
        ...
        <div className=" rounded-lg pe-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <IconButton
              ...
              icon={icons.plusIcon}
              onClick={() => {
                navigate("/user/accounts/sale/create-sale-order");
              }}
            />
          </div>
        </div>
