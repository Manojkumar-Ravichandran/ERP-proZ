...
export default function CreateSalesOrder() {
    ...
    //         gst_percentage: item.gst_slab,
    //         hsn_code: item.hsncode,
    //         tax_amount: item.gst_amount,
    //         total_tax_amount: item.total_price_gst,
    ...
    const breadcrumbItems = [
        ...
        { id: 2, label: "Order", link: "/user/accounts/sale/sale-order" },
        { id: 3, label: location?.state ? "Edit Sale Order" : "Create Sale Order" }
    ];
    ...
    const addMasterHandler = async (data) => {
        ...
        if (location.state) {
            ...
            apiEndpoint = updateSaleOrderEffect(payload); // Ensure this function is available
        } else {
            const payload = {
            ...
            apiEndpoint = createSaleOrderEffect(payload); // Ensure this function is available
        }
        ...
        try {
            ...
            if (response?.data?.status === "success") {
                ...
                navigate('/user/accounts/sale/sale-quotation');
            } else {
                setToastData({
                    show: true,
                    message: response?.data?.message,
