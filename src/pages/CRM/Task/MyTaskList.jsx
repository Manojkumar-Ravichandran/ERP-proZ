...
import { getMyTaskListInProgress, getTaskListInProgress } from "../../../redux/CRM/Task/TaskAction";
...
import { getActivityQueryListEffect, getActivityReplayListEffect, getEmployeeListEffect } from "../../../redux/common/CommonEffects";
import { arrOptForDropdown, getDefaultDate, getDefaultDateTime } from "../../../utils/Data";
import {
  deleteTaskEffect,
  editTaskListEffect,
  getMyTaskListEffect,
...
import { createLeadActivityEffect, getLeadSourceListEffect, getLeadStageListEffect } from "../../../redux/CRM/lead/LeadEffects";
...
export default function MyTaskList() {
  ...
  const taskDatas = useSelector((state) => state?.crmTask?.myTaskList);
...
const datafeed = (event) => {
  ...
  useEffect(() => {
    getTaskList();
  }, [searchText, filterData,paginationCurrentPage]);

  const getTaskList = (payload = {}) => {
    dispatch(getMyTaskListInProgress({
...
, []);
  ...
  const updateTaskHandler = async (data) => {
    ...
    try {
      ...
      setIsUpdateModalOpen(false);
      dispatch(getMyTaskListInProgress({ page: paginationCurrentPage }));
      updateReset();
    }
  };
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        console.log(mapToOptions(stages.data.filter(({ name }) => name !== "New")))
